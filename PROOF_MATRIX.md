...
| Polymarket paper runtime can run offline through the agency entrypoint and persist through the same canonical trade/state files. | Experimental but offline-proven | `python3 tests/destructive/trading-agency-polymarket-test.py` and `python3 tests/trade-schema-contract-test.py` | Paper only; does not prove live readiness or canonical promotion. |
| Mixed mode exists as a limited evaluation path with Hyperliquid preferred for the single entry admitted per cycle. | Limited | `python3 tests/destructive/trading-agency-mixed-test.py` and `python3 tests/destructive/mixed-mode-integration-test.py` | Not presented as a fully proven dual-entry runtime. |
