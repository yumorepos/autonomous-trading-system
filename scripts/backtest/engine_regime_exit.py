...
from config.regime_thresholds import HIGH_FUNDING_MIN_MAX_APY, REGIME_EXIT_THRESHOLDS
...
class BacktestEngine:
    ...
    def __init__(
        ...
        # Regime state (market-wide). Matches live system hysteresis:
        # enter HIGH_FUNDING at max_apy >= 1.50, exit when max_apy < 1.20.
        self.current_regime = "HIGH_FUNDING"
        self.high_funding_entry = HIGH_FUNDING_MIN_MAX_APY
        self.high_funding_exit = REGIME_EXIT_THRESHOLDS["HIGH_FUNDING"]["max_funding_apy"]
    ...
    def _update_regime(self, state: MarketState) -> None:
        """Update market-wide regime with hysteresis from funding rates."""
        if not state.funding_rates:
            return
        max_apy = max(abs(r) * 3 * 365 for r in state.funding_rates.values())
        if self.current_regime == "HIGH_FUNDING":
            if max_apy < self.high_funding_exit:
                self.current_regime = "BELOW_HIGH"
        else:
            if max_apy >= self.high_funding_entry:
                self.current_regime = "HIGH_FUNDING"

    def _check_regime_exit(self, state: MarketState) -> None:
        """If regime is no longer HIGH_FUNDING, close all open positions."""
        if self.current_regime == "HIGH_FUNDING":
            return
        for asset in list(self.positions.keys()):
            pos = self.positions[asset]
            ...
            if pos.direction == "short":
                ...
                hour_funding = self.cost_model.funding_earned(pos.size_usd, -funding_rate, 1.0)
            self._close_position(pos, candle["close"], state.timestamp, "REGIME_EXIT", hour_funding)

    def _check_exits(self, state: MarketState) -> None:
        """Check all open positions for exit triggers."""
        # Regime-exit takes precedence (matches live paper_trader behavior).
        self._update_regime(state)
        self._check_regime_exit(state)
        for asset in list(self.positions.keys()):
            pos = self.positions[asset]
            candle = state.prices.get(asset)
            if not candle or not candle.get("close"):
...
def main() -> None:
    ...
    reason_counts = Counter(t.exit_reason for t in result.closed_trades)
    print("\n==================================================")
    print("EXIT REASONS")
    print("==================================================")
    for reason, count in sorted(reason_counts.items(), key=lambda x: -x[1]):
        pnl = sum(t.net_pnl for t in result.closed_trades if t.exit_reason == reason)
        print(f"{reason:<20} {count:>4}  net=${pnl:>7.2f}")
